...
class QubitGrid:
    ...
    def __init__(self, ds: xr.DataArray, grid_names: list = None, size: int = 3, is_3d: bool = False):
        if grid_names:
        ...
        subplot_kwargs = {"projection": "3d"} if is_3d else {}
        #subplot_kwargs = {"projection": "3d"} 
        figure, all_axes = plt.subplots(*shape, figsize=(shape[1] * size, shape[0] * size), squeeze=False, subplot_kw=subplot_kwargs)
        axes = []
